  ...
  registrarUsuario(event: Event) {
    ...
    this.http.post('http://localhost:8020/auth/register', userData).subscribe({
      next: (response:any) => {
        ...
        this.mensajeRegistro = 'Registro exitoso. Redigiendo a la página del login.';

        setTimeout(() => {

          this.router.navigate(['/login']); // Redirige a la página de login después de un registro exitoso, vamos en caso de que este todo dpm.
        }, 1000);
      },
      error: (error) => {
        if (error.status === 400) {
